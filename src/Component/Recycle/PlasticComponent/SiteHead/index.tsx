...
const DashboardSiteHead = (props: any) => {
  ...
  const renderHeader = (section, _, isActive) => {
    return (
      <Animatable.View
        ...
        <Image
          style={styles.historyHeaderComponent}
  ...
  const postWastedSummaryApi = async (date, date1, date2) => {
    ...
    // @ts-ignore 
    const result = await ApiClient.createApiClient().siteHeadWasteSummary(payload);
    // @ts-ignore 
    if (result.status && result.data.status === true) {
      // @ts-ignore 
      ...
      console.log("newdata123", wastedcollected1, wastedprocessed1);
    }
    else {
      setWastedCollected1([]);
      setWastedProcessed1([]);
  ...
  const CollectiontrendApi = async (date, date1, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadCollectedGraph(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
      if ((result?.data?.data ?? []).length > 0) {
  ...
  const Processedtrendgraph = async (date, date1, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadProcessedGraph(payload);
  ...
  const Segregatedtrendgraph = async (date, date1, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadSegregatedGraph(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
      if ((result?.data?.data ?? []).length > 0) {
...
// eslint-disable-next-line no-unused-vars
  const postWastedProcessedApi = async (date, date1, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadProcessedSummary(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
  ...
  const postproductsummaryapi = async (date, date1, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadProductSummary(payload);
    // @ts-ignore
...
// eslint-disable-next-line no-unused-vars
  const collectedWasteLeftMenuApi = async (date5, date6, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadCollection(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
      if ((result?.data?.data ?? []).length > 0) {
  ...
  const segregationWasteLeftMenuApi = async (date7, date8, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadSegregation(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
      if ((result?.data?.data ?? []).length > 0) {
  ...
  const processedWasteLeftMenuApi = async (date9, date10, date2) => {
    ...
    const result = await ApiClient.createApiClient().siteHeadProcessed(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
      if ((result?.data?.data ?? []).length > 0) {
  ...
  const exportCollectionDataToExcelDownload = (downloadCollectionData) => {
    ...
    RNFS.writeFile(path + '/recycle_plastic_site_head_collection_data_' + new Date().getTime() + '.xlsx', wbout, 'ascii').then(() => {
      {
        Platform.OS === 'android' ?
  ...
  const exportCollectionDataToExcelSharing = (sharingCollectionData) => {
    ...
    // Write generated excel to Storage
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    RNFS.writeFile(path + '/recycle_plastic_site_head_collection_data_sharing.xlsx', wbout, 'ascii').then(() => {
    }).catch((e) => {
      console.log('Error', e);
    });
  ...
  const shareCollectionExcel = async () => {
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    const dir = path + '/recycle_plastic_site_head_collection_data_sharing.xlsx';
  ...
  const exportSegregationDataToExcelDownload = (downloadSegregationData) => {
    ...
    // Write generated excel to Storage
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    RNFS.writeFile(path + '/recycle_plastic_site_head_segregation_data_' + new Date().getTime() + '.xlsx', wbout, 'ascii').then(() => {
      {
        Platform.OS === 'android' ?
        ToastAndroid.showWithGravityAndOffset("Excel File Is Downloaded Successfully", ToastAndroid.SHORT, ToastAndroid.BOTTOM, 25, 50) :
        Alert.alert("Excel File Is Downloaded Successfully");
  ...
  const exportSegregationDataToExcelSharing = (sharingSegregationData) => {
    ...
    // Write generated excel to Storage
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    RNFS.writeFile(path + '/recycle_plastic_site_head_segregation_data_sharing.xlsx', wbout, 'ascii').then(() => {
    }).catch((e) => {
      console.log('Error', e);
    });
  ...
  const shareSegregationExcel = async () => {
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    const dir = path + '/recycle_plastic_site_head_segregation_data_sharing.xlsx';
  ...
  const exportProcessedDataToExcelDownload = (downloadProcessedData) => {
    ...
    // Write generated excel to Storage
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    RNFS.writeFile(path + '/recycle_plastic_site_head_processed_data_' + new Date().getTime() + '.xlsx', wbout, 'ascii').then(() => {
      {
        Platform.OS === 'android' ?
        ToastAndroid.showWithGravityAndOffset("Excel File Is Downloaded Successfully", ToastAndroid.SHORT, ToastAndroid.BOTTOM, 25, 50) :
        Alert.alert("Excel File Is Downloaded Successfully");
  ...
  const exportProcessedDataToExcelSharing = (sharingProcessedData) => {
    ...
    // Write generated excel to Storage
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    RNFS.writeFile(path + '/recycle_plastic_site_head_processed_data_sharing.xlsx', wbout, 'ascii').then(() => {
    }).catch((e) => {
      console.log('Error', e);
    });
    shareProcessedExcel();
  ...
  const shareProcessedExcel = async () => {
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    const dir = path + '/recycle_plastic_site_head_processed_data_sharing.xlsx';
  ...
  const historyApi = async (date3, date4, date2) => {
    ...
    payload.append("end", time1);
    payload.append("siteName", city);
    const result = await ApiClient.createApiClient().siteHeadHistory(payload);
    // @ts-ignore
    if (result.status && result.data.status === true) {
      // @ts-ignore
      if ((result?.data?.data ?? []).length > 0) {
  ...
  const exporthistoryDataToExcelDownload = (downloadHistoryData) => {
    ...
    // Write generated excel to Storage
    const path = Platform.OS === 'ios' ? RNFS.DocumentDirectoryPath : RNFS.DownloadDirectoryPath;
    RNFS.writeFile(path + '/recycle_plastic_site_head_history_data_' + new Date().getTime() + '.xlsx', wbout, 'ascii').then(() => {
      {
        Platform.OS === 'android' ?
        ToastAndroid.showWithGravityAndOffset("Excel File Is Downloaded Successfully", ToastAndroid.SHORT, ToastAndroid.BOTTOM, 25, 50) :
        Alert.alert("Excel File Is Downloaded Successfully");
        ...
        <View style={styles.ThirdContainer}>
          <View style={styles.ThirdcardContainer}>
            ...
            {rdf.length > 0 ? <View style={styles.newdata}>
              ...
              />
            </View> : <View style={styles.detailsNotFoundSummaryView}>
              <Text style={styles.detailsNotFoundSummaryText}>{'Data Is Not Found.'}</Text>
            </View>}

          </View>
        </View>
        <View style={[styles.ThirdContainer, { marginTop: 35 ,marginBottom:50}]}>
          <View style={styles.ThirdcardContainer}>
            <View style={styles.thirdcardFirstOneView}>
...
const styles = StyleSheet.create({
  ...
  detailsNotFoundSummaryView: {
    height: height / 10,
    justifyContent: "center",
    alignItems: "center",
  },
  detailsNotFoundSummaryText: {
    textAlign: "center",
    fontSize: 18,
    color: "#2D2D2D",
  },
