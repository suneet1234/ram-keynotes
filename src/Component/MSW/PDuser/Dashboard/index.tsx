...
const DashboardPdUser = (props: any) => {
  const { route, user, dashboardProcessingData, dashboardDistributeData } = props;
  const { params } = route;
  ...
  const [productcataloguevalue, setProductCatalogueValue] = useState([]);
  ...
  const tabButton = (processData, distributeData) => {
        ...
        <View style={styles.tabButtonContainer}>
          <View>
            <View style={styles.tabView1}>
              ...
              <View style={styles.tabView3}>
              </View>
              <View style={styles.tabView4}>
              </View>
              <View style={styles.tabView5}>
                <Text style={styles.tabText}>Weight(MT)</Text>
              </View>
            </View>
          </View>
          <ScrollView>
          ...
          <View>
            <View style={styles.listView1}>
              ...
              <View style={styles.listView3}>
                <Text style={styles.listText}>Compost{'\n'}   (MT)</Text>
              </View>
              <View style={[styles.viewJustifyContent, { flex: 0.5 }]}>
                <Text style={styles.listText}> RDF{'\n'}(MT)</Text>
              </View>
              <View style={[styles.viewJustifyContent, { flex: 0.8 }]}>
                <Text style={styles.listText}>Recyclables {'\n'}     (MT)</Text>
              </View>
              <View style={[styles.viewJustifyContent, { flex: 0.4 }]}>
                <Text style={styles.listText}>Inerts{'\n'} (MT)</Text>
              </View>
            </View>
          </View>
          <ScrollView>
            {/* @ts-ignore */}
            <Processing processData={processData} />
          </ScrollView>
        </View>
      );
    } else if (filter === "Distribution") {
      return (<View style={styles.tabButtonContainer}>
        <View>
          <View style={styles.distributionView1}>
            <View style={[styles.viewJustifyContent, { flex: 0.6 }]}>
              <Text style={[styles.listText, { marginLeft: 5 }]}>Date</Text>
            </View>
            <View style={[styles.viewJustifyContent, { height: height / 17, width: width / 5 }]} >
              <Text style={[styles.listText, { textAlign: "center" }]}> Compost  Outflow{"\n"} (MT)</Text>
            </View>
            ...
            <View style={[styles.viewJustifyContent, { flex: 0.6 }]}>
              {Platform.OS === 'ios' ?
                <Text style={styles.listText}>   Inerts  Outflow{"\n"}    (MT)</Text> :
                <Text style={styles.listText}>   Inerts {"\n"}  Outflow{"\n"}    (MT)</Text>
              }
            </View>
          </View>
        ...
        <ScrollView>
          {/* @ts-ignore */}
          <Distribution distributeData={distributeData} />
        </ScrollView>
      </View>
      );
    }
  ...
  const calledApis = async () => {
    ...
    await loadDefaultCatalogue();
  };
  // ************************Default Data Methods***********************************
  const loadDefaultHistory = () => {
    historyApi(moment(new Date()).subtract(6, 'days'), moment(new Date()), "date");
  };
  const loadDefaultCatalogue = () => {
    productCatalogue(moment(new Date()).subtract(4, 'days'), moment(new Date()), "date");
  };
  ...
  // ***********************API Methods******************
  // ***********************MSW Summary Or Product Catalogue API Methods******************
  ...
  const productCatalogue = async (date, date1, date2) => {
    ...
    if (result.status && result.data.status === true) {
      // @ts-ignore
      const productcatalogue = result.data.data[0];
      //@ts-ignore
      const catalogue = Object.entries(productcatalogue).map(([key, value]) => {
        return {
          title: key,
          value: value,
        };
      ...
      //  @ts-ignore
      setProductCatalogueValue(catalogue);
      var array1 = [];
      var array2 = [];
      var index = 0;
  ...
  useEffect(() => {
    if (check && !refresh) {
      if (datetype == "Date") {
        ...
        productCatalogue(date, date1, "date");
        ...
        productCatalogue(value1, value2, "month");
        ...
        productCatalogue(value3, value4, "year");
      }
    }
  }, [datetype, refresh]);
  ...
  useEffect(() => {
    ...
    setCheck1(true);
    calledApis();
  }, [dashboardProcessingData, dashboardDistributeData]);
            ...
            <View style={styles.secondTwocardmainView}>
              ...
              >
                ...
                <VictoryBar
                  ...
                  y="totalWaste"
                />
              </VictoryChart> : <View style={styles.chartV1}><Text style={styles.chartV1Text}>{'Data Is Not Found.'}</Text></View>}
            </View>
            <View style={styles.secondThreecardmaibView}>
              <View style={styles.weightinmeticview}>
                <Text style={styles.weightmeticText}>Weight in Metric Ton</Text>
    ...
    return (
      <ScrollView>
        <View style={styles.SecondContainer}>
          <Swiper
            autoplay={false}
            paginationStyle={styles.paginationTrendStyle}
                ...
                <View style={styles.secondTwocardmainView}>
                  ...
                  >
                    ...
                    <VictoryBar
                      ...
                      y="compost"
                    />
                  </VictoryChart> : <View style={styles.chartV1}>
                    <Text style={styles.chartV1Text}>{'Data Is Not Found.'}</Text></View>}
                </View>
                <View style={styles.secondThreecardmaibView}>
                  <View style={styles.weightinmeticview}>
                    <Text style={styles.weightmeticText}>Weight in Metric Ton</Text>
              ...
              <View style={styles.SecondcardContainer}>
                ...
                <View style={styles.secondTwocardmainView}>
                  {distributerecyclablevalue.length > 0 ? <VictoryChart
                    ...
                    theme={VictoryTheme.material}
                    domainPadding={{
                      x: distributerecyclablevalue.length < 3 ? 160 : distributerecyclablevalue.length < 5 ? 85 : 8,
                    }}
            ...
            <View style={styles.reminderModalView1}>
              <View style={styles.reminderTextView}>
                {user?.noFilledDataDates?.filter(i => {
                  return i.module === 'MSW-Processing';
                }).length ?
                ...
                {user?.noFilledDataDates?.filter(i => {
                  return i.module === 'MSW-Distribute';
                }).length ?
                  <Text style={[styles.reminderInputText, { marginTop: 12 }]}>Please input the data for distribute</Text> : null}
                {user?.noFilledDataDates?.filter(i => {
                  return i.module === 'MSW-Distribute';
                }).map(d => {
                  return <Text key={d} style={styles.reminderDateText}>{moment(d.date).format("DD/MM/YYYY")}</Text>;
        ...
        <View style={styles.firstContainer}>
          <View style={styles.firstcardContainer}>
            ...
            <View style={styles.secondcardmainView}>
              {tabButton(dashboardProcessingData, dashboardDistributeData)}
            </View>
          </View>
        </View>
        <View>
        ...
        <View style={styles.ThirdContainer}>
          <View style={styles.ThirdcardContainer}>
            <View style={styles.thirdcardFirstOneView}>
              <Text style={styles.processedwasteText}>
                Processed Waste (MTD)
              </Text>
            </View>
            {productcataloguevalue.length > 0 ? <View style={styles.thirdcardFirstTwoView}>
              <SwiperFlatList
                autoplay={false}
...
const styles = StyleSheet.create({
  ...
  chartV1Text: { textAlign: "center", fontSize: 18, color: "#2D2D2D" },
  ThirdcardContainer: {
    height: height / 4.4,
    backgroundColor: "#F8F8F8",
    width: width / 1.06,
  ...
  dataNotFoundTrendText: {
    textAlign: "center",
    fontSize: 18,
    color: "#2D2D2D",
  },
  paginationTrendStyle: {
    position: 'absolute',
    top: height / 2.5,
    right: 10,
    bottom: 0,
